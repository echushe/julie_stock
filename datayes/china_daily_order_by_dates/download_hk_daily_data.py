...
def download_all_tickers(asset_class='E'):
    url = '/api/master/getSecID.json?field=&assetClass={}&ticker=&partyID=&cnSpell=&listStatusCD=L,S,DE,UN&exchangeCD=XHKG'.format(asset_class)
    print(url)
...
def download_multi_ticker_data_of_a_date(ticker_list, date, assert_class='E'):
    ...
    if assert_class == 'E':
        url = '/api/market/getMktHKEqud.json?field=&beginDate=&endDate=&secID=&ticker={0}&tradeDate={1}'.format(tickers_as_str, date.replace("-", ""))
    elif assert_class == 'IDX':
        url = '/api/market/getMktIdxdHsi.json?field=&beginDate=&endDate=&ticker={0}&tradeDate={1}'.format(tickers_as_str, date.replace("-", ""))
...
def download_one_day_data(tickers_id_as_key, date, root_dir, asset_class='E'):
    ...
    if asset_class == 'E':
        root_dir = os.path.join(root_dir, 'hk_daily_stock_order_by_dates')
    elif asset_class == 'IDX':
        root_dir = os.path.join(root_dir, 'hk_daily_index_order_by_dates')
    else:
        raise NotImplementedError('Error: asset_class {} is not supported'.format(asset_class))
...
def download_hk_multi_day_data(first_date, last_date, root_dir, asset_class='E'):
...
if __name__ == '__main__':
    ...
    download_hk_multi_day_data(args.first_date, args.last_date, args.root_dir, asset_class=args.asset_class)
